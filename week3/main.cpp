...
public:
    ...
    void intput()
    {
        ...
        if(k==0)
        {
            ...
            if(tmp[length-2]!='^'&&tmp[length-1]!='x')//³£Êý
            {
                fans=stof(tmp);
                num[0]+=fans;
    ...
    void newton()
    {
        duoxiangshi two(*this);//二阶导函数
        duoxiangshi one(*this);//一阶导函数
        int isok=1;//判断变号
        double x=b;
        one.qiudao();
        two.qiudao();
        two.qiudao();
        if(result(a)*result(b)>0||result(a)*result(b)==0)//条件一
        {
            cout<<"error";
            return;
        }
        ...
        if(one.result(a)*one.result(b)<=0)
        {
            cout<<"error";
            return;
        }
        isok=two.result(x)>0?1:-1;
        while(true)
        {
            if(result(x)==0)
            {
            ...
            if((two.result(x)>0&&isok<0)||(two.result(x)<0&&isok>0))
            {
                cout<<"error";
                return;
            }
            x=x-result(x)/one.result(x);
            if((result(x)/one.result(x)>0&&result(x)/one.result(x)<0.000001)||(result(x)/one.result(x)<0&&result(x)/one.result(x)>-0.000001))
            {
                shuchu(x);
                return;
            }
